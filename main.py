...
async def word_in_dict(word: str) -> bool:
    headers = {'apikey': SUPABASE_KEY, 'Authorization': f'Bearer {SUPABASE_KEY}'}
    async with httpx.AsyncClient() as c:
        try:
            r = await c.get(
...
def _unreg() -> str:
    return random.choice([
        ...
        "🃏 *GameMaster:* \"Unregistered souls are invisible. DM me, register, then come back.\"",
    ])
...
async def cmd_fusion(message: types.Message):
    ...
    if not get_user(str(message.from_user.id)):
        await message.answer("🃏 *GameMaster:* \"Unregistered triggering my game. Bold, stupid. DM me — but fine, I'll start.\"\n\n_(DM me to register!)_", parse_mode="Markdown")
...
async def cmd_inventory(message: types.Message):
    ...
    for item in inv:
        ...
        if   "wood"   in itype and "crate" in itype: lbl, cb = f"🪵 WOOD CRATE ({xp} XP)",   f"open_{iid}"
        elif "bronze" in itype and "crate" in itype: lbl, cb = f"🥉 BRONZE CRATE ({xp} XP)", f"open_{iid}"
        elif "iron"   in itype and "crate" in itype: lbl, cb = f"⚙️ IRON CRATE ({xp} XP)",   f"open_{iid}"
        elif "super"  in itype and "crate" in itype: lbl, cb = f"🎁 SUPER CRATE ({xp} XP)",  f"open_{iid}"
        elif itype == "shield":                       lbl, cb = "🛡️ SHIELD — tap to ACTIVATE", f"activate_shield_{iid}"
        elif itype == "teleport":                     lbl, cb = "🌀 TELEPORT",                 f"teleport_{iid}"
        elif "multiplier" in itype:
            mult = item.get('multiplier_value', 2)
            kind = "XP" if "xp" in itype else "SILVER"
            lbl, cb = f"⚡ {kind} MULTIPLIER x{mult}", f"use_{iid}"
        elif "locked_" in itype:                      lbl, cb = "🔒 LEGENDARY [TOO POWERFUL]", f"info_{iid}"
        else:                                         lbl, cb = f"❓ {itype.upper()}",          f"use_{iid}"
...
async def cmd_claims(message: types.Message):
    ...
    for item in unclaimed:
        ...
        # callback carries the item's unique id
        rows.append([InlineKeyboardButton(text=lbl, callback_data=f"claim_{iid}")])

    # Auto-claim all button
    rows.insert(0, [InlineKeyboardButton(text="⚡ CLAIM ALL", callback_data="claim_all")])

    await message.answer(
        f"🎁 *UNCLAIMED REWARDS*\n━━━━━━━━━━━━━━━━━━━\n"
        f"⚠️ {len(unclaimed)} item(s) waiting!\n"
        f"💡 Tap *[CLAIM]* or *[CLAIM ALL]*",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=rows),
        parse_mode="Markdown"
    )
...
async def cmd_autoclaim(message: types.Message):
    ...
    if not get_user(u_id):
        await message.answer("🃏 *GameMaster:* \"No account. Register first.\"", parse_mode="Markdown"); return
    await _do_claim_all(message, u_id)
...
async def cb_claim(query: types.CallbackQuery):
    ...
    if not remaining:
        ...
        # Rebuild the claims list in-place
        item_labels = {
            "xp_multiplier":     lambda m: f"⚡ XP MULTIPLIER x{m}",
            "silver_multiplier": lambda m: f"💎 SILVER MULTIPLIER x{m}",
            "super_crate":  lambda _: "🎁 SUPER CRATE",
        ...
        rows = [[InlineKeyboardButton(text="⚡ CLAIM ALL", callback_data="claim_all")]]
...
async def _do_use_item(message: types.Message, user_id: str, item_id: int):
    ...
    if not item: await message.answer("🃏 *GameMaster:* \"Invalid item.\"", parse_mode="Markdown"); return
    itype = item.get('type','').lower()
    if "crate"     in itype: await message.answer("🃏 *GameMaster:* \"Use the OPEN button for crates.\"", parse_mode="Markdown")
    elif itype == "shield":  await message.answer("🃏 *GameMaster:* \"Tap the ACTIVATE SHIELD button in your inventory.\"", parse_mode="Markdown")
    elif "locked_" in itype: await message.answer("🃏 *GameMaster:* \"You can't use that. Upgrade backpack first.\"", parse_mode="Markdown")
    elif "multiplier" in itype:
        mult = item.get('multiplier_value', 2)
        kind = "XP" if "xp" in itype else "SILVER"
        # Activate multiplier
    ...
    else: await message.answer("🃏 *GameMaster:* \"Unknown item.\"", parse_mode="Markdown")


async def _do_claim_all(target, user_id: str, edit: bool = False):
    """Claim all unclaimed items that fit in inventory."""
    unclaimed = get_unclaimed_items(user_id)
    if not unclaimed:
        txt = "🃏 *GameMaster:* \"You have no unclaimed rewards. Go earn something.\""
        if edit: await target.edit_text(txt, parse_mode="Markdown")
        else:    await target.answer(txt, parse_mode="Markdown")
        return

    profile = get_profile(user_id)
    available = (profile.get('backpack_slots', 5) - profile.get('inventory_count', 0)) if profile else 0
    if available <= 0:
        txt = (f"🃏 *GameMaster:* \"Your pathetic fannypack is FULL.\"\n\n"
               f"📦 Slots: {profile['inventory_count']}/{profile['backpack_slots']}\n"
               f"Use or open items first.")
        if edit: await target.edit_text(txt, parse_mode="Markdown")
        else:    await target.answer(txt, parse_mode="Markdown")
        return

    claimed = 0
    for item in list(unclaimed)[:available]:
        ok, _ = claim_item(user_id, item['id'])
        if ok: claimed += 1

    remaining = get_unclaimed_items(user_id)
    txt = (f"✅ *AUTO-CLAIM COMPLETE!*\n━━━━━━━━━━━━━━━━━━━\n"
           f"🎁 Items claimed: {claimed}\n")
    if remaining:
        txt += f"⚠️ {len(remaining)} item(s) couldn't fit (inventory full)"
    else:
